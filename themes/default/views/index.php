...
<?php
			...
			foreach ($friends as $member) {
?>
			<div class="chat" data-chat="person<?php echo $i; ?>">
				<div class="chat-history">
					...
					<ul>
						<?php Comments\Handling::view($member['id']); ?>
					</ul>
				</div>

				<form class="form-horizontal chat-message clearfix" method="post" action="<?php echo $linksDir . 'messages/' . $member['id']; ?>">
					<input class="<?php echo $i; ?>" type="text" id="message-to-send" name="message-to-send" placeholder="<?php echo $clauses->get('message_placeholder'); ?>" required>

					<button id="send" name="send" class="pull-right button-send"><?php echo $clauses->get('send'); ?></button>
